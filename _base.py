...
class BaseTriggerModel(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, X_probas, y, cost_matrices=None,X_val=None,X_probas_val=None,y_val=None,X_test=None,X_probas_test=None,y_test=None):
        ...
        if self.require_classifiers:
            if X_probas_val is not None:
                self._fit(X_probas,y,X_val,X_probas_val,y_val,X_test,X_probas_test,y_test) # calimera one regressor scenario DQN model to monitor the loss on the test set
            else:
